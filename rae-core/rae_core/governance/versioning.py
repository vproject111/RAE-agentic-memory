...
class GitHubFlowStrategy(BaseValidationStrategy):
    ...
    def validate(self, branch_name: str) -> Tuple[bool, str]:
        ...
        if branch_name.startswith("release/") or branch_name.startswith("hotfix/"):
            prefix, version = branch_name.split("/", 1)
            if version.startswith('v'):
                return False, f"Release/Hotfix branch '{branch_name}' uses legacy 'v' prefix. SemVer branch names must omit 'v'."
            if not self.SEMVER_REGEX.match(version):
                return False, f"Version '{version}' in branch '{branch_name}' does not comply with SemVer 2.0.0."
        return True, ""
...
class VersioningValidator:
    def __init__(self, project_path: str, module_name: str, config: Optional[dict] = None):
    ...
    def _load_config(self) -> dict:
        ...
        env_strict = (
            os.getenv("RAE_STRICT_SEMVER", "false").lower() == "true" or
            os.getenv("GIT_FLOW_STRICT", "false").lower() == "true"
        )
        if env_strict:
            strategy = "git-flow"
            strict = True
        ...
        if os.path.exists(pyproject_path):
            try:
                ...
                with open(pyproject_path, "rb") as f:
                    data = tomllib.load(f)
                    version_config = data.get("tool", {}).get("rae", {}).get("versioning", {})
