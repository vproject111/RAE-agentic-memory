...
class CapabilityMatrix(BaseModel):
    ...
    json_schema: bool = Field(
        default=False, description="Supports structured outputs / JSON mode"
    )
    tools: bool = Field(
        default=False, description="Supports tool calling / function calling"
    )
    ...
    streaming: bool = Field(
        default=True, description="Supports chunked/streamed token output"
    )
    reasoning: bool = Field(
        default=False, description="Has advanced reasoning/thinking capacity"
    )
...
class ModelDescriptor(BaseModel):
    ...
    context_window: int = Field(
        default=128000, description="Delineated input context size in tokens"
    )
