...
class AgentOutputContract(BaseModel):
    ...
    analysis: str = Field(
        ..., min_length=10, description="Deep reasoning behind the decision"
    )
    decision: AgentDecision = Field(..., description="Selected action")
    confidence: float = Field(..., ge=0.0, le=1.0, description="Confidence score")
    retrieved_sources: list[str] = Field(
        default_factory=list, description="IDs of memories used as evidence"
    )
    pattern_used: str | None = Field(
        None, description="The agentic pattern used (e.g., Double-Check)"
    )
