...
class TestInMemoryStorageCoverage:
    ...
    async def test_count_memories_all_filters(self):
        ...
        await storage.store_memory(content="m1", layer=layer, tenant_id=t, agent_id=a)
        await storage.store_memory(content="m2", layer="l2", tenant_id=t, agent_id=a)
        await storage.store_memory(
            content="m3", layer=layer, tenant_id=t, agent_id="a2"
        )
    ...
    async def test_adjust_importance_missing_field(self):
        ...
        mem_id = await storage.store_memory(
            content="m", layer="l", tenant_id="t", agent_id="a"
        )
    ...
    async def test_update_memory_layer_explicit(self):
        ...
        storage = InMemoryStorage()
        mem_id = await storage.store_memory(
            content="m", layer="old_layer", tenant_id="t", agent_id="a"
        )
    ...
    async def test_clear_tenant_with_tags(self):
        ...
        storage = InMemoryStorage()
        await storage.store_memory(
            content="c", layer="l", tenant_id="t1", agent_id="a1", tags=["tag1"]
        )
    ...
    async def test_delete_memory_with_tags(self):
        ...
        storage = InMemoryStorage()
        mem_id = await storage.store_memory(
            content="c", layer="l", tenant_id="t", agent_id="a", tags=["tag1", "tag2"]
        )
...
class TestInMemoryVectorStoreCoverage:
    ...
    async def test_delete_vector_wrong_tenant(self):
        ...
        store = InMemoryVectorStore()
        mem_id = await store.store_memory(tenant_id="t1")
        await store.store_vector(mem_id, [1.0], "t1")

        success = await store.delete_vector(mem_id, "t2")
        assert success is False
        assert mem_id in store._vector_indices["default"]

    @pytest.mark.asyncio
    async def test_batch_store_exception(self):
        """Test batch store ignoring invalid vectors."""
        store = InMemoryVectorStore()
        mem_id1 = await store.store_memory(tenant_id="t1")
        mem_id2 = await store.store_memory(tenant_id="t1")
        ...
        assert count == 1
        assert mem_id1 in store._vector_indices["default"]
        assert mem_id2 not in store._vector_indices["default"]
