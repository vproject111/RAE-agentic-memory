    ...
    def render(self):
        with ui.column().classes("w-full max-w-7xl mx-auto p-8 gap-y-6"):
            create_module_header(
                "Hive Sandboxes & Worktrees",
                "Isolated OCI container runtimes, SBOM validations, and resources cgroups.",
                self.client,
            ...
            with ui.row().classes("w-full gap-6"):
                ...
                with ui.card().classes("w-full p-6 bg-slate-900 text-white shadow-sm"):
                    ui.label("ACTIVE SANDBOX RUNTIMES").classes(
                    ...
                    with ui.column().classes("w-full gap-y-4").props("role=list"):
                        for sbx in self.sandboxes:
                            ...
                            with (
                                ...
                                .props(
                                    'tabindex=0 role="listitem" aria-label="Sandbox runtime: click for details"'
                                )
                                .on(
                                    "click",
                                    lambda s_title=f"Sandbox {sbx['id']}", s_det=card_details: (
                                        self.on_inspect(s_title, s_det)
                                        if self.on_inspect
                                        else None
                                    ),
                                )
                                ...
                                with ui.row().classes("gap-4"):
                                    ...
                                    ui.label(sbx["container"]).classes(
                                        "font-mono font-bold text-slate-200"
                                    )
                                    ui.label(f'CPU: {sbx["cpu_usage"]}').classes(
                                        "text-xs text-slate-400"
                                    )
                                    ui.label(f'Memory: {sbx["mem_usage"]}').classes(
                                        "text-xs text-slate-400"
                                    )
                                with ui.row().classes("gap-2"):
                                    ui.badge(sbx["manifest_status"]).props(
                                        "color=green-9"
                                    )
                                    ui.badge(sbx["sbom_signature"]).props(
                                        "color=blue-9"
                                    )
