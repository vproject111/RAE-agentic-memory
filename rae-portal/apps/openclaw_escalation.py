...
class OpenClawEscalationApp:
    ...
    def authorize_override(self, inc_id):
        ...
        if not safe_reason:
            ui.notify(
                "Błąd: Uzasadnienie jest wymagane do autoryzacji obejścia (ISO 27001 / GDPR).",
                type="negative",
            )
            return
        ...
        audit_hash = (
            "SHA256:8f4c9c2b3e8a1d7f0e9d8c7b6a5f4e3d2c1b0a9f8e7d6c5b4a3f2e1d0c9b8a7f"
        )
        ui.notify(
    ...
    def deny_rollback(self, inc_id):
        ui.notify(
            f"Odrzucono obejście. Uruchamianie automatycznego wycofania zmian dla {inc_id}...",
            type="info",
        )
    ...
    def render(self):
        with ui.column().classes("w-full max-w-7xl mx-auto p-8 gap-y-6"):
            create_module_header(
                "OPENCLAW ESCALATION DESK",
                "Manual gate overrides, AST blocks, and constitutional bypass logs.",
                self.client,
                rls_status="ESCALATED",
            )
            ...
            with ui.row().classes("w-full gap-6"):
                ...
                with ui.card().classes(
                    ...
                    ui.label("ACTIVE TRIBUNAL INCIDENTS").classes(
                    ...
                    with ui.column().classes("w-full gap-y-4"):
                        for inc in self.incidents:
                            ...
                            def click_handler(
                                c_title=f"Incident {inc['id']}", c_det=card_details
                            ):
                                if self.on_inspect:
                            ...
                            with (
                                ui.card()
                                .classes(
                                    "w-full p-4 bg-slate-800 border-l-4 border-red-600 cursor-pointer hover:bg-slate-700 focus:outline focus:outline-2 focus:outline-blue-500"
                                )
                                .props(
                                    'tabindex=0 aria-label="Incident Details: '
                                    + inc["id"]
                                    + '"'
                                )
                                ...
                                with ui.row().classes(
                                    ...
                                    with ui.column().classes("gap-y-1"):
                                        ui.label(inc["id"]).classes(
                                            "text-sm font-bold text-red-200"
                                        )
                                        ui.label(inc["level"]).classes(
                                            "text-xs text-red-300 font-bold"
                                        )
                                        ui.label(inc["desc"][:80] + "...").classes(
                                            "text-xs text-slate-300"
                                        )
                                    ui.badge(inc["source"]).props(
                                        f'color=red-9 aria-label="Źródło: {inc["source"]}"'
                                    )

                # Manual Gate Controls
                with ui.card().classes(
                    "flex-grow p-6 bg-slate-900 text-white border border-slate-800 shadow-sm"
                ):
                    ui.label("MANUAL INTERVENTION GATE").classes(
                        "text-xs font-bold text-red-200 mb-4"
                    )
                    with ui.column().classes("w-full gap-y-4"):
                        ui.label(
                            "Resolve incident INC-100 (C6 Path Invariant)"
                        ...
                        reason_input = (
                            ...
                            .props('dark required aria-required="true"')
                        )
                        reason_input.on(
                            "change",
                            lambda: setattr(
                                self, "override_reason", reason_input.value
                            ),
                        )
                        ...
                        with ui.row().classes("w-full gap-4"):
                            ui.button(
                                "AUTHORIZE BYPASS",
                                on_click=lambda: self.authorize_override("INC-100"),
                            ).props("color=red rounded")
                            ui.button(
                                "DENY & ROLLBACK",
                                on_click=lambda: self.deny_rollback("INC-100"),
                            ).props("outline color=slate rounded")
