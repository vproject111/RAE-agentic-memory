...
class SemanticResonanceEngine:
    ...
    def compute_resonance(
        ...
        If a retrieved memory is highly connected to other high-scoring memories,
        its resonance increases.
        """
        if not initial_results or not graph_edges:
            return initial_results

        # 1. Map memory IDs to scores
        id_to_score = {r['id']: r.get('search_score', 0.5) for r in initial_results}
        resonance_boosts = {r['id']: 0.0 for r in initial_results}

        # 2. Spread energy through edges (Simple PageRank-like push)
        for u, v, weight in graph_edges:
            if u in id_to_score and v in id_to_score:
                # Mutual reinforcement
                resonance_boosts[u] += id_to_score[v] * weight * self.resonance_factor
                resonance_boosts[v] += id_to_score[u] * weight * self.resonance_factor

        # 3. Update results with boosted scores
        for r in initial_results:
            original_score = r.get('math_score') or r.get('search_score', 0.0)
            boost = resonance_boosts.get(r['id'], 0.0)
